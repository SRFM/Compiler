...
def lex():
    ...
    verbal_unit = []        #Empty some_list (filled with characters later...)
    rtrn_verbal_unit = []   #My return some_list

    # --- A some_list of reserved Starlet words: --- #
...
#----------------------------------------------------------------------------------------------------------#
global quad_list    # List containing all the quads  
quad_list = []      # Empty global some_list of all the quads used by the program
...
def genquad(op, v1, v2, v3):
    ...
    some_list = []
    some_list = [nextquad()]                      # adding quad number
    some_list += [op] + [v1] + [v2] + [v3]
    # some_list.append([op] + [v1] + [v2] + [v3])   # adding arguments

    quad_ID += 1            # next_quad = current_quad + 1
    quad_list += [some_list]
    # quad_list.append(some_list)     # adding current some_list to the some_list with all the quads used by the program
    # print("size: ", len(quad_list))
    return some_list
...
def newtemp():
    ...
    some_list = ['T_']
    some_list.append(str(T_x))
    tmp_var = "".join(some_list)
    T_x += 1

    # Adding the temporary variables in a some_list
    tmp_variables_list += [tmp_var]

    ent = Entity()
    ent.type = 'TEMP'
    ent.name = tmp_var
    ent.tempVar.offset = calculate_offset()
    new_entity(ent)

    return tmp_var

# Subroutine emptylist(): creates an empty some_list of quad labels #
def emptylist():
    label_list = []

    return label_list

# Subroutine makelist(x): creates a some_list of quad labels including only "x" #
def makelist(x):
    current_list = [x]

    return current_list

# Subroutine mergelist(l1, l2): creates a some_list of quad labels merging the two lists #
def mergelist(list_1, list_2):
    some_list = []
    some_list += list_1 + list_2

    return some_list

# Subroutine backpatch(some_list, z): searches for quads with void last variable and fills them with the "z" label #
def backpatch(some_list, z):
    global quad_list

    for i in range(len(some_list)):
        for j in range(len(quad_list)):
            if (some_list[i] == quad_list[j][0] and quad_list[j][4] == '_'):
                quad_list[j][4] = z
                j = len(quad_list)
    return
...
#----------------------------------------------------------------------------------------------------------#
# --- Triangle: --- #
# ---We used the Magic Method of Python---
class Argument():
    def __init__(self):
...
# --- Rectangle: --- #
class Entity():
    #Using the Magic method of Python __init__() God Bless the internet!
    def __init__(self):
        self.name = ''
        self.type = ''
...
# Adding current object to the some_list: #
def new_argument(object):
    global arguments, entities

    arguments.append(object)       #Adding the object (obj) to my some_list.
...
def new_scope(name):
    ...
    entities = []                   #Emptying the entities some_list in order to renew them
...
def calculate_offset():
    ...
    if (scopes[-1].entity_list is not []):
        for entity in (scopes[-1].entity_list):
            if (entity.type == 'VAR' or entity.type == 'TEMP' or entity.type == 'PARAM' or entity.type == 'SUBPR'):
                counter += 1
...
def add_parameters():
    ...
    for arg in arguments:
        ...
        ent.name = arg.name
        ent.type = 'PARAM'
        ent.parameter.mode = arg.parameterization
    ...
    for s in scopes:
        ...
        for e in s.entity_list:
            if (e.type == 'VAR'):
                ...
                print("\tEntity: " + " Name: " + e.name + "\t Type: " + e.type + "\t Mode: " + e.parameter.mode + "\t Offset: " + str(e.parameter.offset))
    print("--------------------------------------------------------------------------------------------------")

# ----------------------------------------------------------------------------------------------------------- #
#                                 ---  SYNTAX ANALYST & INTERMEDIATE CODE & FINAL CODE ---                    #
# ----------------------------------------------------------------------------------------------------------- #

def final_touches(assembly_file):
    global quad_list
    # print ("LENGTH OF QUAD", str(len(quad_list)))
    ...
    def subprogram():
        ...
        if (token[0] == 'function'):
            ...
            if (token[1] == 1):     # tokenID
                name = token[0]     # Keep the name of the function
...
def int_stuff(int_file):
    for i in range(len(quad_list)):
        quad = quad_list[i]
        int_file.write(str(quad[0]))
        int_file.write(": ")
        int_file.write(str(quad[1]))
        int_file.write(" ")
        int_file.write(str(quad[2]))
        int_file.write(" ")
        int_file.write(str(quad[3]))
        int_file.write(" ")
        int_file.write(str(quad[4]))
        int_file.write("\n")
...
def my_files():
    ...
    assembly_file = open('assembly_file.asm', 'w+')
    ...
    assembly_file.write('beq, $t1, $t2, ' + '\n')
    assembly_file.write('bgt, $t1, $t2, ' + '\n')
    assembly_file.write('blt, $t1, $t2, ' + '\n')
    assembly_file.write('bne, $t1, $t2, ' + '\n')
    assembly_file.write('bge, $t1, $t2, ' + '\n')
    assembly_file.write('ble, $t1, $t2, ' + '\n')
    assembly_file.write('+, $t1, $t1, $t2' + '\n')
    assembly_file.write('-, $t1, $t1, $t2' + '\n')
    assembly_file.write('*, $t1, $t1, $t2' + '\n')
    assembly_file.write('/, $t1, $t1, $t2' + '\n')
    assembly_file.write('li $v0, 1' + '\n')
    assembly_file.write('li $a0, ' + '\n')
    assembly_file.write('Syscall')
